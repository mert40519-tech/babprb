...
async def adm_iban_ok(call: CallbackQuery, bot: Bot):
    ...
    d = await one("SELECT * FROM deals WHERE id=?", (did,))
    # Alıcıya: ödemen onaylandı, ürünü onayla
    try:
        await bot.send_message(
            d["buyer_id"],
            f"✅ <b>Ödemeniz Onaylandı!</b>\n\n"
            ...
            f"📦 Ürün/hizmet teslim alındığında aşağıdaki butona basın.\n"
    ...
    # Satıcıya: ödeme alındı, teslimat bekleniyor
    try:
        await bot.send_message(
            d["seller_id"],
            f"🔔 <b>Ödeme Doğrulandı!</b>\n\n"
            f"Anlaşma: <b>#{d['code']}</b>\n"
            f"Alıcının ödemesi admin tarafından onaylandı.\n\n"
            f"⏳ Ürün/hizmeti teslim edin — alıcı onayladıktan sonra ödemeniz yapılacak."
        )
    except: pass
    try: await call.message.edit_text("✅ IBAN ödemesi onaylandı! Taraflara bildirim gönderildi.")
...
async def adm_iban_done(call: CallbackQuery, bot: Bot):
    if not is_admin(call.from_user.id):
        await call.answer("🚫", show_alert=True); return
    did = int(call.data.split(":")[1])
    d = await one("SELECT * FROM deals WHERE id=?", (did,))
    # Satıcıya: paranız gönderildi bildirimi
    if d:
        fee = round(d["amount"] * FEE_PERCENT / 100, 2)
        net = round(d["amount"] - fee, 2)
        try:
            await bot.send_message(
                d["seller_id"],
                ...
                f"✅ Para hesabınıza aktarılmıştır."
            )
        except Exception: pass
        # Alıcıya: ürünü onaylaması için bildirim
        try:
            await bot.send_message(
                d["buyer_id"],
                f"📦 <b>Ürününüzü Teslim Aldınız mı?</b>\n\n"
                f"Anlaşma: <b>#{d['code']}</b>\n"
                f"Tutar: <b>{d['amount']} {d['currency']}</b>\n\n"
                f"Ürünü/hizmeti aldıysanız lütfen onaylayın:",
                reply_markup=ikb(
                    [("✅ Evet, Teslim Aldım", f"release:{did}")],
                    [("⚠️ Sorun Var — Dispute Aç", f"dispute:{did}")]
                )
            )
        except Exception: pass
    try: await call.message.edit_text("✅ IBAN ödemesi gönderildi olarak işaretlendi. Taraflara bildirim yapıldı.")
    except: pass
    await call.answer()

@admin_r.message(StateFilter(Adm.send_to))
async def adm_send_to(msg: Message, state: FSMContext, bot: Bot):
    if not is_admin(msg.from_user.id): return
    if msg.text == "❌ İptal":
        await state.clear()
        await msg.answer("❌ İptal.", reply_markup=main_kb(msg.from_user.id)); return
...
async def adm_do_send(msg: Message, state: FSMContext, bot: Bot = None):
    ...
    if tx:
        ...
        # Satıcıya: para geldi bildirimi
        if bot and "ca_id" in data:
            ca = await one("SELECT * FROM crypto_addr WHERE id=?", (data["ca_id"],))
            if ca:
                d = await one("SELECT * FROM deals WHERE id=?", (ca["deal_id"],))
                if d:
                    try:
                        await bot.send_message(
                            d["seller_id"],
                            ...
                            f"🔗 TX: <code>{tx}</code>\n\n"
                            f"✅ Ödeme cüzdanınıza aktarılmıştır."
                        )
                    except Exception: pass
                    # Alıcıya: ürünü onayla bildirimi
                    try:
                        await bot.send_message(
                            d["buyer_id"],
                            f"📦 <b>Ürününüzü Teslim Aldınız mı?</b>\n\n"
                            ...
                            f"Ürünü/hizmeti aldıysanız lütfen onaylayın:",
                            reply_markup=ikb(
                                [("✅ Evet, Teslim Aldım", f"release:{d['id']}")],
                                [("⚠️ Sorun Var — Dispute Aç", f"dispute:{d['id']}")]
                            )
                        )
                    except Exception: pass
    else:
