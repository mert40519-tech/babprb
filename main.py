...
# Grup yönetici onay süresi (saat) — bu süre sonra grup adminleri onaylayabilir
...
# Coin normalizer: USDT → USDT_TRC20
...
async def db_init() -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        -- Escrow anlaşmaları (sadece bakiye tabanlı, kripto)
...
def gen_code() -> str:
    return secrets.token_hex(6).upper()  # 12 karakter
...
def coin_display(coin: str) -> str:
    """USDT_TRC20 → USDT gibi kısa görüntü"""
    c = normalize_coin(coin)
...
async def format_deal_message(d: Dict) -> str:
    """İstenen formatta escrow mesajı üret"""
...
# ═══════════════════════════════════════════════════════════
#  /tic — Hızlı escrow: /tic 24 USDT @kullanici açıklama
# ═══════════════════════════════════════════════════════════
...
async def cmd_tic(msg: Message, state: FSMContext, bot: Bot) -> None:
    ...
    # /ticaret onay/iptal/sorgula KOD işlemleri
    ...
    # /tic 24 USDT @kullanici açıklama
    # Parse: miktar, coin, @mention, açıklama
    amount   = None
    coin     = None
    seller_id   = None
    seller_name = None
    ...
    # Argümanları parse et (mention dışındaki kelimeler)
    non_mention_parts = []
    for i, p in enumerate(parts[1:], 1):
        if p.startswith("@"): continue
        # entity offset kontrolü
    ...
    # Gruba veya DM'e gönder
    await msg.answer(deal_msg)

    # Satıcıya DM bildirim (grup değilse zaten görüyor)
    if is_group(msg):
        try:
...
async def _handle_ticaret_sub(msg: Message, bot: Bot, uid: int, sub: str, code: str) -> None:
    ...
    # onay — sadece alıcı veya grup admin (deadline sonrası)
    if sub == "onay":
        if d["status"] != "confirmed":
            await msg.answer(f"❌ Bu anlaşma zaten <b>{d['status']}</b> durumunda.")
            return

        # Grup admin kontrolü (deadline sonrası)
        ...
        # Ödeme: komisyon master'a, kalanı satıcıya
        fee = round(d["amount"] * FEE_PERCENT / 100, 8)
        net = round(d["amount"] - fee, 8)

        # Satıcı bakiyesine ekle
        await add_balance(d["seller_id"], d["coin"], net)
        # Komisyon log (master cüzdana aktarıldı kabul)
    ...
    # iptal — alıcı veya her iki taraf rızasıyla
    if sub == "iptal":
        if d["status"] != "confirmed":
        ...
        # İptal için karşı taraf onayı gerekiyor mu?
        # Eğer alıcı iptal ediyorsa → direkt
        if uid == d["buyer_id"] or is_admin(uid):
            ...
            # Satıcı iptal istiyorsa — iptal talebi oluştur
...
async def wallet_deposit_btn(call: CallbackQuery) -> None:
    """Buton handler — doğrudan deposit menüsü göster."""
    if is_group(call.message):
        await call.answer("Sadece DM'de kullanılır.", show_alert=True)
        return
...
async def wallet_wd_btn(call: CallbackQuery, state: FSMContext) -> None:
    """Buton handler — uid call.from_user'dan alınmalı, call.message.from_user bot olur."""
    uid  = call.from_user.id
    bals = await all_balances(uid)
    if not bals:
